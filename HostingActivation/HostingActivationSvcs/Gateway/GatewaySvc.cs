...
{
    public class GatewaySvc : StatelessService
    {
        public GatewaySvc()
        {
            Task.Factory.StartNew(async () =>
            {
                ...
                var ServiceName = "GatewaySvc";
                var processID = Process.GetCurrentProcess().Id.ToString();
                var partitionID = ServiceInitializationParameters.PartitionId.ToString();
                var replicaId = ServiceInitializationParameters.InstanceId.ToString(); // since this is a statless service, we are instances 
